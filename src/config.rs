...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("remote_hub".to_owned());
...
impl Config {
    ...
    pub fn get_rendezvous_server() -> String {
        let mut rendezvous_server = FLUTTER_CONFIG_SERVER.read().unwrap().clone();
        ...
        log::info!("loading rendezvous server {rendezvous_server}");
        rendezvous_server
    }

    pub fn get_rs_key() -> String {
        let key = FLUTTER_CONFIG_KEY.read().unwrap().clone();
        if !key.is_empty() {
            return key;
        }
    ...
    pub fn set_flutter_config_key(key: String) {
        *FLUTTER_CONFIG_KEY.write().unwrap() = key;
    }

    pub fn set_flutter_config_server(server: String) {
        *FLUTTER_CONFIG_SERVER.write().unwrap() = server;
    }

    pub fn get_rendezvous_servers() -> Vec<String> {
        let s = FLUTTER_CONFIG_SERVER.read().unwrap().clone();
        if !s.is_empty() {
            log::info!("Loading server from flutter config: {s}");
            return vec![s];
        }

        let s = EXE_RENDEZVOUS_SERVER.read().unwrap().clone();
        if !s.is_empty() {
            log::info!("Loading server from exe config: {s}");
            return vec![s];
        }
        let s = Self::get_option("custom-rendezvous-server");
        if !s.is_empty() {
            log::info!("Loading server from custom config: {s}");
            return vec![s];
        }
        let s = PROD_RENDEZVOUS_SERVER.read().unwrap().clone();
        if !s.is_empty() {
            log::info!("Loading server from PROD config: {s}");
            return vec![s];
        }
        ...
        if serial_obsolute {
            let ss: Vec<String> = Self::get_option("rendezvous-servers")
                ...
                .collect();
            if !ss.is_empty() {
                log::info!("Loading server from option config: {ss:?}");
                return ss;
            }
        }
        let servers = RENDEZVOUS_SERVERS.iter().map(|x| x.to_string()).collect();
        log::info!("Loading server from RENDEZVOUS_SERVERS: {servers:?}");
...
impl LocalConfig {
    ...
    pub fn get_device_id() -> String {
        LOCAL_CONFIG.read().unwrap().device_id.clone()
    }
